...
running_connector_count() {
  docker ps --format '{{.Names}}' | awk '$0 == "cliproxyapi-setup-cloudflared-1" || $0 == "shaman-api-cloudflared-1" {count++} END {print count+0}'
}

target_is_exclusive() {
  local target="$1" cpa=0 litellm=0
  docker ps --format '{{.Names}}' | grep -qx 'cliproxyapi-setup-cloudflared-1' && cpa=1 || true
  docker ps --format '{{.Names}}' | grep -qx 'shaman-api-cloudflared-1' && litellm=1 || true
  if [[ "$target" == cpa ]]; then
    (( cpa == 1 && litellm == 0 ))
  else
    (( litellm == 1 && cpa == 0 ))
...
for seed in 7101 7102; do
  for target in litellm cpa; do
    ...
    while kill -0 "$pid" 2>/dev/null; do
      if (( $(running_connector_count) > 1 )); then
        observed_overlap=1
      fi
      sleep 0.1
    done
    ...
    jq -nc \
      ...
      --argjson connector_overlap "$observed_overlap" \
      '{transition:$transition,seed:$seed,target:$target,status:"pass",recovery_ms:$recovery_ms,connector_overlap:$connector_overlap}' \
      >>"$samples"
...
overlaps = sum(sample["connector_overlap"] for sample in samples)
successes = sum(sample["status"] == "pass" for sample in samples)
summary = {
    "evaluation": "EVAL-005",
    "status": "pass" if len(samples) == 4 and successes == 4 and overlaps == 0 and max(durations) <= 120 and samples[-1]["target"] == "cpa" else "fail",
    "transition_count": len(samples),
    "transition_success_rate": successes / 4,
    "connector_overlap_count": overlaps,
    "recovery_seconds": {
