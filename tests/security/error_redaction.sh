...
: "${CPA_MANAGEMENT_KEY:?CPA_MANAGEMENT_KEY is required in .env}"
: "${CPAMP_ADMIN_KEY:?CPAMP_ADMIN_KEY is required in .env}"
: "${CPA_TUNNEL_TOKEN:?CPA_TUNNEL_TOKEN is required in .env}"
...
cpa_status="$(curl -sS --max-time 15 -o "$tmp/cpa-response.json" -w '%{http_code}' \
  -H "Authorization: Bearer $invalid" "$CPA_LOCAL_BASE_URL/v1/models")"
...
for secret in "${secret_values[@]}"; do
  if grep -Fq -- "$secret" "$tmp/inspection.txt"; then
    printf 'credential disclosure detected in response or logs\n' >&2
    exit 1
  fi
done
...

{
  ...
  printf 'known_secret_values_checked=%s\n' "${#secret_values[@]}"
