...
timeout 120 bash -c 'until curl -fsS http://127.0.0.1:8317/healthz 2>/dev/null >/dev/null; do sleep 1; done'

docker compose up -d --no-deps --force-recreate cpa-manager-plus
timeout 120 bash -c 'until curl -fsS http://127.0.0.1:18317/health 2>/dev/null | jq -e ".ok == true" >/dev/null; do sleep 1; done'
admin_key="$CPAMP_ADMIN_KEY"
timeout 120 bash -c 'until curl -fsS -H "Authorization: Bearer '"$admin_key"'" http://127.0.0.1:18317/status 2>/dev/null | jq -e ".collector.collector == \"running\" and (.collector.lastError // \"\") == \"\"" >/dev/null; do sleep 1; done'

python3 scripts/render-public-config.py
docker compose up -d --no-deps --force-recreate cpamp-public
timeout 120 bash -c 'until curl -fsS http://127.0.0.1:18417/health 2>/dev/null | jq -e ".ok == true" >/dev/null; do sleep 1; done'
