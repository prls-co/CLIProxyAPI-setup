...
All commands run from the repository checkout. The ignored mode-`0600` `.env`
is the single canonical source for manually managed secrets, keys, and
configuration variables. `.env.local` and `state/secrets/` are obsolete and
unsupported. Protected files under `state/` are generated runtime state; never
paste `.env` or protected state into commands, tickets, or logs. This setup
...
Bootstrap creates `.env` from `.env.example` when needed and
...
request reached the active machine; set `CPA_ORIGIN_HOSTNAME` in `.env` to
...
ignored `.env`; the token must have Zone Read, DNS Edit, and Cloudflare
...
load_env
curl -fsS -H "Authorization: Bearer $CPAMP_ADMIN_KEY" http://127.0.0.1:18317/status | jq .
unset CPAMP_ADMIN_KEY
```

The public dashboard uses CPA Manager Plus's native admin-key login. The
renderer and Compose both consume `CPAMP_ADMIN_KEY` from the canonical `.env`.
Caddy does not add a second authentication layer or inject authorization.
